...
class MixerModulev2(BaseModule):
    def __init__(self,
                 num_channels=768,
                 norm_cfg=dict(type='LN', eps=1e-6, requires_grad=True),
                 dropout_layer=None,
                 init_cfg=None):
        super(MixerModulev2, self).__init__(init_cfg)
...
class ViTMixAdapterv5(VisionTransformer):
    def __init__(self,
                 img_size=224,
                 patch_size=16,
                 in_channels=3,
                 ...
                 num_interactions=4,
                 align_corners=False):
        super(ViTMixAdapterv5, self).__init__(img_size,
                                              patch_size,
                                              in_channels,
                                              embed_dims,
                                              num_layers,
        ...
        self.adap_interactions = ModuleList(MixerModulev2(embed_dims,
        ...
        self.adpt_feature_upsamples = ModuleList([nn.Upsample(scale_factor=2,
                                                              mode='bilinear',
                                                  ...
                                                  nn.Upsample(scale_factor=8,
                                                              mode='bilinear',
                                                              align_corners=self.align_corners)])
        self.adap_feature4_generator = nn.Conv2d(3 * embed_dims,
                                                 embed_dims,
                                                 kernel_size=1)
    ...
    def forward(self, inputs):
        ...
        pyramid_features.insert(0, self.adap_feature4_generator(
            torch.cat(tuple(upsample(feature)
                            for feature, upsample in zip(pyramid_features,
                                                         self.adpt_feature_upsamples)), dim=1)))
