...
class ResBlockNormal(BaseModule):
    def __init__(self,
                 in_channels=768,
                 channels=768,
                 ...
                 dropout_layer=None,
                 init_cfg=None):
        super(ResBlockNormal, self).__init__(init_cfg)
...
class MixerModulev6(BaseModule):
    def __init__(self,
                 num_channels=768,
                 ...
                 dropout_layer=None,
                 init_cfg=None):
        super(MixerModulev6, self).__init__(init_cfg)
        self.mix_linear = nn.Linear(25, 25)

        self.dropout_layer = build_dropout(
            dropout_layer) if dropout_layer else torch.nn.Identity()

        self.vit_value = nn.Conv2d(num_channels,
                                   num_channels,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1,
                                   groups=num_channels)
        self.f8_value = nn.Conv2d(num_channels,
                                  num_channels,
                                  kernel_size=3,
                                  stride=1,
                                  padding=1,
                                  groups=num_channels)
        self.f16_value = nn.Conv2d(num_channels,
                                   num_channels,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1,
                                   groups=num_channels)
        self.f32_value = nn.Conv2d(num_channels,
                                   num_channels,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1,
        ...
        self.ffn = ResBlockNormal(in_channels=num_channels,
    ...
    def forward(self, vit_feature, feature8, feature16, feature32):
        ...
        vit_feature = nchw2nlc2nchw(self.vit_norm, self.vit_value(vit_feature))
        feature8 = nchw2nlc2nchw(self.f8_norm, self.f8_value(feature8))
        feature16 = nchw2nlc2nchw(self.f16_norm, self.f16_value(feature16))
        feature32 = nchw2nlc2nchw(self.f32_norm, self.f32_value(feature32))
...
class ViTMixAdapterv13(VisionTransformer):
    def __init__(self,
                 img_size=224,
                 patch_size=16,
                 in_channels=3,
                 ...
                 num_interactions=4,
                 align_corners=False):
        super(ViTMixAdapterv13, self).__init__(img_size,
                                               patch_size,
                                               in_channels,
                                               embed_dims,
                                               num_layers,
        ...
        self.adap_interactions = ModuleList(MixerModulev6(embed_dims,
