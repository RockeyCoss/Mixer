...
class CFFN(BaseModule):
    def __init__(self,
        ...
        self.feedforward_channels = embed_dims * expand_ratio
...
class Injection(BaseModule):
    def __init__(self,
                 ...
                 dropout_layer=None,
                 init_cfg=None):
        super(Injection, self).__init__(init_cfg)
        self.inject_linear = nn.Linear(25, 4)
    ...
    def forward(self, vit_feature, feature8, feature16, feature32):
        ...
        aligned_feature = torch.cat((vit_feature,
                                    ...
                                    dim=4)
        # B, H, W, C, 4
        vit_feature = self.inject_linear(aligned_feature)
...
class Extraction(BaseModule):
    def __init__(self,
                 num_channels=768,
                 norm_cfg=dict(type='LN', eps=1e-6, requires_grad=True),
                 dropout_layer=None,
                 init_cfg=None):
        super(Extraction, self).__init__(init_cfg)
        self.extract_linear = nn.Linear(25, 21)
        ...
        self.ffn = CFFN(num_channels, 4, dropout_layer=dropout_layer)

    def forward(self, vit_feature, feature8, feature16, feature32):
        # Their shapes are B, C, H, W
        feature8_i, feature16_i, feature32_i = \
            feature8, feature16, feature32
        ...
        aligned_feature = torch.cat((vit_feature,
                                    ...
                                    dim=4)
        # B, H, W, C, 21
        aligned_feature = self.extract_linear(aligned_feature)
        ...
        return [feature8, feature16, feature32]


@BACKBONES.register_module()
class ViTMixAdapterv10(VisionTransformer):
    def __init__(self,
                 img_size=224,
                 patch_size=16,
                 in_channels=3,
                 ...
                 num_interactions=4,
                 align_corners=False):
        super(ViTMixAdapterv10, self).__init__(img_size,
                                               patch_size,
                                               in_channels,
                                               embed_dims,
                                               num_layers,
        ...
        self.adap_injections = ModuleList(Injection(embed_dims,
                                                    dropout_layer=dict(type='DropPath',
                                                                       drop_prob=inter_dpr[i]))
                                          for i in range(self.num_interactions))
        self.adap_extractions = ModuleList(Extraction(embed_dims,
                                                      dropout_layer=dict(type='DropPath',
                                                                         drop_prob=inter_dpr[i]))
                                           for i in range(self.num_interactions))

        self.extraction_indexes = [(num_layers // num_interactions) * i - 1
                                   for i in range(1,
                                                  self.num_interactions + 1)]
        self.injection_indexes = [i - 1 for i in self.extraction_indexes]
    ...
    def forward(self, inputs):
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i in self.injection_indexes:
                ...
                vit_feature = self.adap_injections[
                    self.injection_indexes.index(i)](vit_feature, *pyramid_features)
                vit_feature = nchw_to_nlc(vit_feature)
                if self.with_cls_token:
                    x = torch.cat((x[:, 0: 1], vit_feature), dim=1)

            if i in self.extraction_indexes:
                if self.with_cls_token:
                    # Remove class token and reshape token for decoder head
                    vit_feature = x[:, 1:]
                else:
                ...
                vit_feature = vit_feature.reshape(B, hw_shape[0], hw_shape[1],
                                                  C).permute(0, 3, 1, 2).contiguous()
                pyramid_features = self.adap_extractions[
                    self.extraction_indexes.index(i)](vit_feature, *pyramid_features)
